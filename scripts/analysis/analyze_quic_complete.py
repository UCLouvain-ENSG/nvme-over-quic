...
if len(sys.argv) > 1:
    ...
    log_file = os.path.join(BUILD_DIR, "client_quic_single.log")
...
if not os.path.exists(log_file):
    ...
    print(f"Example: {sys.argv[0]} client_quic_single.log", file=sys.stderr)
    sys.exit(1)

# Track I/O queue (qid=1) commands with timestamps
io_sends = []  # [(timestamp, cid, line_num, line), ...] - only qid=1 sends
io_completes = []  # [(timestamp, cid, line_num, line), ...] - completions
all_lines = []  # [(timestamp, line_num, line), ...] - all lines with timestamps for gap analysis

# Patterns for QUIC
# Send: nvme_quic_qpair_capsule_cmd_send with "Opened stream for req cid=X" on qid:1
send_pattern = re.compile(r'\[([\d\-: .]+)\].*qid:1.*Opened stream for req cid=(\d+)')
# Complete: nvme_complete_request with opc=02 (READ opcode)
...
for send_idx, (send_ts, send_cid, send_line_num, send_line) in enumerate(io_sends):
    ...
    # Don't assume order - search all completions
    found = False
    best_match = None
    best_match_idx = -1
