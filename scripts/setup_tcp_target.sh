#!/usr/bin/env bash
#
# SPDK TCP NVMe-oF Target Setup Script
# This script configures an NVMe-oF target with TCP transport
...
echo "==> Setting up SPDK NVMe-oF Target with TCP transport"

# Create TCP transport
echo "==> Creating TCP transport..."
sudo "$RPC_PY" nvmf_create_transport -t $TRANSPORT -u 16384 -m 8 -c 8192
if [ $? -ne 0 ]; then
    echo "ERROR: Failed to create TCP transport"
    exit 1
fi
...
# Add TCP listener
echo "==> Adding TCP listener at $LISTEN_ADDR:$LISTEN_PORT"
...
echo "    -r 'trtype:TCP adrfam:IPv4 traddr:$LISTEN_ADDR trsvcid:$LISTEN_PORT subnqn:$SUBSYSTEM_NQN'"
echo ""
